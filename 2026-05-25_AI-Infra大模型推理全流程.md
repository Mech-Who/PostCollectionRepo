...
category: AI技术
depth: 深度
tags: [AI, 大模型, LLM推理, vLLM, PagedAttention, ContinuousBatching, 显存优化, Transformer, GPU优化]
summary: 深度拆解LLM推理全流程，以Llama 3为例逐层分析Tensor维度变化，详解Continuous Batching与PagedAttention两大高性能推理基石，覆盖Tokenize到Sampling的完整推理链路。
...
> **摘要：** 作者基于vLLM源码深入阅读后的系统性总结，以Llama 3为例追踪推理过程中每一步的张量维度变化。核心涵盖Continuous Batching（token-level调度）和PagedAttention（虚拟页表机制）两大技术原理，以及从Tokenize、Embedding、Transformer Block（RMSNorm/Fused QKV/RoPE/FlashAttention/FFN）到LM Head、Sampling的完整推理流程拆解。

## 我的理解
> 由小林生成，供小涵审阅修改

这是我看过最清晰的大模型推理流程拆解之一。作者抓住了两个关键洞察：

1. **Trace tensor shapes**——追踪推理每一步的维度变化是理解LLM最好的方式，这比读论文有效得多。从flattened token数组 `[num_sched_tokens, hidden_size]` 到attention阶段的请求级隔离，shape的变化揭示了一切。

2. **Continuous Batching + PagedAttention 是当今LLM高性能推理的基石**——vLLM将调度从request level下沉到token level，配合虚拟页表管理KV Cache，解决了"请求输出长度不可预测"这一生成式任务的核心矛盾。

特别有价值的部分：
- **Prefill vs Decode 的计算特性差异**：Prefill是GEMM（计算密集型），Decode是GEMV（访存密集型），Continuous Batching的价值在于让多个请求复用权重，将GEMV重新变回GEMM
- **Attention的Scale问题**：Decode阶段每个请求读取独占的KV Cache，无法通过跨请求共享摊薄访存成本，这是长上下文推理的核心瓶颈
- **vLLM v0 vs v1的差异**：v0中prefill走独立分支(kernel读临时buffer)，v1统一为paged cache + block_table，代价是prefill多一次间接寻址，换来prefix cache和chunked prefill的天然支持
- **Sampling的Gumbel-Max技巧**：用加噪求最大值替代轮盘赌，将串行前缀和计算变为element-wise并行
- **LM Head最后一层的优化说明**：理论上只要算最后一个token的hidden_state，但vLLM还是全部算完——工程与物理极限的trade-off

这篇文章对从事AI Infra或想深入理解LLM推理细节的人非常有价值。后面提到的后续方向（MLA/DSA、FlashAttention、MoE、TP/PP/DP/EP/SP/CP）也值得继续深入。

## 📌 关键要点
- **核心方法**：通过张量维度变化追踪理解LLM推理流程，比读论文更高效
- **Continuous Batching**：token-level调度替换request-level调度，消除padding浪费，4个硬性约束（最大并发数/token budget/模型最大序列长度/空闲KV Cache blocks）
- **PagedAttention**：虚拟页表机制解决显存碎片问题，block_table + slot_mapping实现间接寻址，默认block_size=16平衡访存效率
- **Prefill阶段**：计算密集型（GEMM），受限于Tensor Cores算力峰值
- **Decode阶段**：访存密集型（GEMV），受限于HBM带宽，需要读取模型权重+KV Cache
- **FlashAttention**：Online Softmax + Kernel Fusion + Tiling，打破内存墙
- **GQA分组共享**：1 KV Head映射给4 Q Head，减少KV Cache显存占用
- **采样优化**：Greedy先行 + Temperature Scaling + Gumbel-Max采样替代轮盘赌
- **MoE本质**：将FFN层拆分为多个"专家"，每次仅激活部分参数
- **并行策略**：TP(列切分/行切分)、PP、DP、EP、SP(Sequence Parallelism)
...
| Q-K Dot Product | 查询Query和该请求所有Key的点积，注意力原始分数。->`[query_lens, num_heads, seq_lens]` |
| Scale | 对原始分数进行数值缩放（ 为向量维度），防止 Softmax 函数进入梯度极小的区域。 |
| Mask | 注入因果逻辑，通过加上掩码矩阵 （通常将未来位置设为 ），阻止模型"看到"未来的信息。 |
| Softmax | 将处理后的分数转化为总和为 1 的注意力权重（概率）。->`[query_lens, num_heads, seq_lens]` |
| MatMul | 根据注意力权重  对值向量（Value）进行加权求和，得到最终的**输出特征表示**。V的Shape为`[seq_lens, num_heads, head_dim]`   -> `[query_lens, num_heads, head_dim]` |
...
| **目标矩阵** | **计算公式** | **输入 维度** | **权重矩阵 维度** | **输出结果维度** | **备注** |
| :--- | :--- | :--- | :--- | :--- | :--- |
| O_Proj | `[num_sched_tokens, hidden_size]` | `[hidden_size, hidden_size]` | `[num_sched_tokens, hidden_size]` | 跨 Head 的信息交互与融合。这也是一次标准的稠密矩阵乘法（GEMM / GEMV）。 |
...
| 目标矩阵 | 计算公式 | 输入 维度 | 输入 维度 | 输出结果维度 | 备注 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Residual Add** | `[num_sched_tokens, hidden_size]` | `[num_sched_tokens, hidden_size]` | `[num_sched_tokens, hidden_size]` |  是**进入这一层 RMSNorm 之前**的原始输入。将注意力机制提取的新特征"加回"主干数据流中。 |
...
| :--- | :--- | :--- | :--- | :--- |
| Gate Proj | `[num_sched_tokens, hidden_size]` | `[hidden_size, intermediate_size]` | `[num_sched_tokens, intermediate_size]` |
| Up Proj | `[num_sched_tokens, hidden_size]` | `[hidden_size, intermediate_size]` | `[num_sched_tokens, intermediate_size]` |
...
| :--- | :--- | :--- | :--- | :--- |
| Fused Gate/Up | `[num_sched_tokens, hidden_size]` | `[hidden_size, 2*intermediate_size]` | `[num_sched_tokens, 2*intermediate_size]` |
...
| 操作名称 | 数学公式 | 物理实现说明 |
| :--- | :--- | :--- |
| Slice & Act |  为  前一半， 为后一半。 |

##### Down_Proj

| 目标矩阵 | 计算公式 | 输入 维度 | 权重 维度 | 输出结果维度 |
| :--- | :--- | :--- | :--- | :--- |
| Down Proj | `[num_sched_tokens, intermediate_size]` | `[intermediate_size, hidden_size]` | `[num_sched_tokens, hidden_size]` |
...
> 更近一步，Prefill阶段Transformer Block的最后一层，attention 输出是 \[num_sched_tokens, hidden_dim\]，但实际上只需要最后一个 token 的 hidden_state 来做 logits 计算（因为之后没有更多的 layer 需要完整的 hidden_states 了）。所以理论上最后一层的MLP（以及LayerNorm）可以只对需要采样的 token 计算，节省大量计算。
>
> 如果只需要最后一个 token 的生成结果，那么在最后一层（Last Layer）：
> 1.  必须要算的部分：所有 token 的  和 （因为这层的 KV cache 要存下来，供后续 Decode 阶段使用）。
> 2.  理论上可以只算最后一个 token： 的投影、Attention 的计算（其实退化成了 Decode 阶段的 Flash-Decoding 计算）、LayerNorm、以及庞大的 MLP。
...
| 目标矩阵 | 计算公式 | 输入 维度 | 权重矩阵 维度 | 输出结果维度 | 备注 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **LM Head** | `[num_reqs, hidden_size]` | `[hidden_size, vocab_size]` | `[num_reqs, vocab_size]` | `num_reqs` 即当前 Batch 中的请求数量。以 Llama-3-8B 为例，`vocab_size` 高达 128256，这是一个极庞大的矩阵，vLLM 通常会使用张量并行（TP）按列切分该权重以加速计算。 |
...
```
  {
    ...
    "top_logprobs": [          // 概率最高的前 N 个 token (用户通过 logprobs=N 指定)
      ...
      {
        "token": "Hi",
        "logprob": -4.32185    // ln(0.013) ≈ -1.32, 即 1.3% 概率
      }
    ]
  }
```
...
2.  Temperature Scaling：`Logits = Logits / Temperature`。
    > : 拉大分数差距。高分越明显，低分被压得更低。模型变得**保守、确定**。: 缩小分数差距。分布趋向均匀。模型变得**随机、有创造力**（但也容易胡说八道）。
    > 对于  的 greedy 请求，此处会用  跳过除法（避免除以零），它们的最终结果由 Step 1 的 `greedy_sampled` 决定。
3.  Argmax-invariant Processors（如 min_p）：
...
5.  Top-K 截断：只保留分数最高的  个 Token，其余得分全部设为 。
6.  Top-P  Masking：按分数从高到低排序并累加 softmax 概率，保留累积概率刚好超过  的前若干 Token，尾部 Token 得分设为 。
7.  Softmax + Gumbel-Max 采样  确保结果的随机性         公式： ，其中
    ...
    ```
8.  最终选择：逐请求按 temperature 决定使用哪个结果
    ```
...
| head_dim/head_size | 128 | 单个头的维度。公式  中的 。 |
...
| intermediate_size | 14336 | FFN 内部升维后的维度（通常为 ）。 |
...
## 相关笔记
- [概念：Harness Engineering] — 这篇文章中关于AI Infra的工程优化思路与Harness Engineering理念相通，都是通过系统级优化（Continuous Batching、PagedAttention、Kernel Fusion）而非暴力计算解决问题
