...
category: AI技术
depth: 深度
tags: [MCP, Function Calling, Agent工程, 协议设计, 工具调用, LLM工程, JSON-RPC]
summary: 系统对比 MCP 协议与 Function Calling 的本质差异——不是 API 形式的区别，而是设计哲学的区别。MCP 解决的是 N×M 工具集成问题（让一份工具实现被任意客户端复用），Function Calling 是点对点的应用内工具约定。包含三层架构、三种传输模式、选型判断框架。
source_url: https://mp.weixin.qq.com/s/OZGXAdEn1OfNcrcjpwTxxQ
source: weixin
status: 📥已采集
sync_status: ❌未同步
---

> **摘要：** 本文深入对比了 MCP 协议和 Function Calling 的本质差异。核心论点是两者的区别不在 API 形式，而在设计哲学——Function Calling 是模型和单个应用之间的点对点约定，工具定义跟应用绑定；MCP 是客户端和服务端之间的标准化协议，工具独立部署为 Server，可以被任意 MCP 客户端复用。文章提出了 USB-C 类比（N×M → M+N），详解了三层架构（Host/Client/Server）和三种传输方式（stdio/SSE/HTTP），并给出了选型判断框架：多客户端共用/跨团队复用/工具需独立生命周期→上MCP；单一应用/临时工具→Function Calling 就够。

## 我的理解
> 由小林生成，供小涵审阅修改

这篇文章的价值在于把 MCP 从"另一个 API"的认知偏差中拉了出来，放到"协议层"的高度来理解。几个让我很有启发的点：

1. **设计哲学差异**：Function Calling 是工具属于应用（私有），MCP 是工具属于自己（独立）。这个归属问题决定了 MCP 必须是协议而不是 API 规范。如果用一句话区分：Function Calling 解决的是"这次调用让模型知道有什么工具"，MCP 解决的是"工具如何独立存在、如何被多个客户端复用"。

2. **N×M → M+N 的 USB-C 类比**：这个类比很有力。没有 MCP 时，N 个模型客户端 × M 个工具 = N×M 次适配；有了 MCP，N+M 就够了。这个数学上的简洁性对面试沟通很有帮助。

3. **选型框架（反例+正例）**：
   - 反例：单一内部 Agent 硬上 MCP，多了通信开销和运维负担
   - 正例 1：公司内部知识库给多个 AI 工具（Claude Code、Cursor、Cline）共用，包成 MCP Server 后改一次三处生效
   - 正例 2：跨团队的工具复用（平台团队做 GitHub MCP Server，全公司复用）

4. **三层架构+三种传输**：Host（应用环境）→ Client（协议适配层）→ Server（独立部署）。传输方式：stdio（本地子进程）、SSE（远程长连接）、HTTP（2025新增简化远程传输）

这篇文章对我们当前项目的参考价值很高——我们正在构建 WorkBuddy 生态，涉及多个 Agent 客户端和工具，MCP 的标准化思路可以直接指导我们自己的工具设计。

## 📌 关键要点

- **核心方法**：将工具调用从"应用内私有调用"提升为"协议化生态"——MCP 基于 JSON-RPC 2.0，以 Host/Client/Server 三层架构实现工具的独立部署和多客户端复用
- **选型框架**：多客户端共用/跨团队复用/工具需独立生命周期→上MCP；单一应用/临时工具→Function Calling 就够
- **传输模式**：stdio（本地子进程通信，开发阶段）、SSE（远程长连接，公司内网共享）、HTTP（2025新增，适合公网部署）
- **适用场景**：Agent 工程面试、工具链架构设计、企业内部 AI 工具标准化
...
工具调用早期就是这个状态。OpenAI 有自己的 Function Calling，Anthropic 有自己的 tool_use，国内的厂商各有各的实现。一个工具想被多个模型调用，得对每家 API 适配一遍。如果一个应用想接入多个工具，每个工具的对接方式还不一样。这个矩阵被官方文档称为 **N×M 问题，** N 个模型客户端 × M 个工具，每对组合都要写一次集成代码。

MCP 做的事情，是把 N×M 变成 **M+N**。

模型客户端只需要实现一次 MCP 协议（变成 N），工具服务方也只需要实现一次 MCP 协议（变成 M），N+M 远小于 N×M。任何客户端连任意服务端，只要两边都遵守 MCP，就能直接用。

这就是为什么 Anthropic 必须把它做成"协议"，不能做成"更好的 Function Calling"。Function Calling 是 OpenAI 的、tool_use 是 Anthropic 的，只要工具调用还是 API 私有规范，就一定有 N×M 的兼容性问题。**只有协议这种形态，才能跨厂商、跨客户端、跨工具地复用工具实现。**
...
## 相关笔记
- concepts/Harness Engineering — MCP 作为协议层是 Harness Engineering 中工具标准化的关键组件
- concepts/AI约束与验证框架 — MCP 的工具声明机制本质上是约束注入的一种形式
